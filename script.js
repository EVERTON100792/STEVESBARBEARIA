...
class BarberPro {
    ...
    switchDashboardView(viewId, isInitialLoad = false) {
        ...
        document.getElementById('dashboardTitle').textContent = newTitle.title;
        document.getElementById('dashboardSubtitle').textContent = newTitle.subtitle;
    ...
    setupEventListeners() {
        ...
        document.getElementById('addComandaBtn')?.addEventListener('click', () => this.showNewComandaModal());
        document.getElementById('bookingForm')?.addEventListener('submit', e => { e.preventDefault(); this.handleAppointmentSubmit(e.target); });
        document.getElementById('loginForm')?.addEventListener('submit', e => { e.preventDefault(); this.handleLoginSubmit(); });
        document.getElementById('demoLoginBtn')?.addEventListener('click', () => this.login('barbeiro', '123'));
        document.getElementById('logoutBtn')?.addEventListener('click', () => this.logout());
        document.getElementById('logoutBtnSidebar')?.addEventListener('click', () => this.logout());
        document.getElementById('refreshBtn')?.addEventListener('click', () => { this.loadData(); this.loadDashboardData(); this.showNotification('Dados atualizados!'); });
        document.getElementById('sidebarToggle')?.addEventListener('click', () => document.querySelector('.dashboard-sidebar')?.classList.toggle('show'));
        document.getElementById('clientSearchInput')?.addEventListener('input', () => this.renderFullClientsList());
        document.getElementById('resetDataBtn')?.addEventListener('click', () => this.resetTestData());
        const agendaDateFilter = document.getElementById('agendaDateFilter');
        if (agendaDateFilter) { agendaDateFilter.addEventListener('change', () => this.renderAgendaView()); }
        document.getElementById('prevDayBtn')?.addEventListener('click', () => this.navigateAgendaDays(-1));
        document.getElementById('nextDayBtn')?.addEventListener('click', () => this.navigateAgendaDays(1));
        ...
        document.getElementById('addExpenseBtnMain')?.addEventListener('click', () => this.showExpenseModal());
        document.getElementById('addAppointmentBtn')?.addEventListener('click', () => this.showAppointmentModal());
        document.getElementById('openCaixaBtn')?.addEventListener('click', () => this.showAbrirCaixaModal());
        document.getElementById('closeCaixaBtn')?.addEventListener('click', () => this.showFecharCaixaModal());
        document.getElementById('viewHistoryBtn')?.addEventListener('click', () => this.showCaixaHistoryModal());
        ...
        document.addEventListener('click', e => {
            ...
            switch (action.dataset.action) {
                ...
                case 'show-payment-modal': this.showPaymentModal(id); break; // Alterado
        ...
        document.getElementById('modalBackdrop').addEventListener('click', (e) => { if (e.target.id === 'modalBackdrop') this.hideModal(); });
    }
    ...
    handleLoginSubmit() { const username = document.getElementById('username').value; const password = document.getElementById('password').value; if (!this.login(username, password)) { this.showNotification('Usuário ou senha inválidos.', 'error'); } }
    ...
    updateStats() { const today = this.getTodayDateString(); const currentMonth = today.slice(0, 7); const todayAppointments = this.appointments.filter(a => a.date === today && (a.status === 'scheduled' || a.status === 'em_atendimento')); const todayRevenue = (this.caixa.entradas || []).filter(e => e.data.startsWith(today)).reduce((sum, e) => sum + e.valor, 0); const monthlyRevenue = this.appointments.filter(a => a.status === 'completed' && a.date.startsWith(currentMonth)).reduce((sum, a) => { const service = this.services.find(s => s.id === a.service); return sum + (service?.price || 0); }, 0); const openComandas = this.comandas.filter(c => c.status === 'aberta').length; document.getElementById('todayCount').textContent = todayAppointments.length; document.getElementById('openComandasCount').textContent = openComandas; document.getElementById('todayProfit').textContent = this.formatCurrency(todayRevenue); document.getElementById('monthlyProfit').textContent = this.formatCurrency(monthlyRevenue); document.getElementById('totalClientsCount').textContent = this.clients.length; const completedThisMonth = this.appointments.filter(a => a.date.startsWith(currentMonth) && a.status === 'completed'); document.getElementById('completedServicesCount').textContent = completedThisMonth.length; const averageTicket = completedThisMonth.length > 0 ? monthlyRevenue / completedThisMonth.length : 0; document.getElementById('averageTicket').textContent = this.formatCurrency(averageTicket); const newClientsThisMonth = this.clients.filter(client => { const firstAppointment = this.appointments.find(a => a.phone === client.phone); return firstAppointment && firstAppointment.createdAt.startsWith(currentMonth); }).length; document.getElementById('newClientsCount').textContent = newClientsThisMonth; }
...
`;
    ...
    showModal(title, content, options = {}) { document.getElementById('modalTitle').textContent = title; document.getElementById('modalContent').innerHTML = content; document.getElementById('modalBackdrop').style.display = 'flex'; const modalContainer = document.getElementById('modalContainer'); modalContainer.style.maxWidth = options.width || '600px'; }
    hideModal() { document.getElementById('modalBackdrop').style.display = 'none'; }
    showNotification(message, type = 'success') { const container = document.getElementById('notificationContainer'); const notification = document.createElement('div'); notification.className = `notification ${type}`; notification.textContent = message; container.appendChild(notification); setTimeout(() => { notification.remove(); }, 4000); }
