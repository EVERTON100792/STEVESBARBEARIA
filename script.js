...
class BarberPro {
    ...
    switchDashboardView(viewId, isInitialLoad = false) { 
        if (!viewId) return; 
        document.querySelectorAll('.content-view').forEach(view => { view.classList.remove('active'); view.style.display = 'none'; }); 
        const viewElement = document.getElementById(viewId); 
        if (viewElement) { viewElement.style.display = 'block'; viewElement.classList.add('active'); } 
        const titles = { 
            ...
            comandasView: { title: 'Atendimentos', subtitle: 'Gerencie os atendimentos pendentes do dia.' }, 
    ...
    checkDataVersion() { 
        const storedVersion = localStorage.getItem('barberpro_data_version'); 
    ...
    login(username, password) { 
        if (username === 'barbeiro' && password === '123') { 
            this.currentUser = { username: 'barbeiro', name: 'Barbeiro Pro' }; 
    ...
    updateStats() { 
        ...
        const pendingCount = this.appointments.filter(a => a.date === todayStr && a.status === 'pending').length; 
        const updateEl = (id, value) => { 
            const el = document.getElementById(id); 
            if (el) el.textContent = value; 
        }; 
        updateEl('todayCount', todayAppointments.length); 
        updateEl('pendingCount', pendingCount); 
        updateEl('todayProfit', this.formatCurrency(todayRevenue)); 
    }
    ...
    renderDashboardAppointments() { 
        ...
        const upcomingAppointments = this.appointments
            .filter(a => a.date >= today && (a.status === 'scheduled' || a.status === 'pending'))
            .sort((a,b) => (a.date + a.time).localeCompare(b.date + b.time)); 
        appointmentsListEl.innerHTML = upcomingAppointments.length === 0 
            ? '<div class="empty-state"><div class="empty-icon">📅</div><p>Nenhum agendamento futuro.</p></div>' 
            : upcomingAppointments.map(a => this.createAppointmentCard(a)).join(''); 
    }
    ...
    createAppointmentCard(appointment) { 
        ...
        if (appointment.status === 'scheduled' || appointment.status === 'pending') { 
            actionButton = `<button class="btn-primary btn-small" data-action="show-finalize-modal" data-id="${appointment.id}">Finalizar Atendimento</button>`; 
        } 
        return `
            <div class="appointment-card status-${appointment.status}">
                ...
                <div class="appointment-footer">
                    ...
                    <div class="appointment-actions">${actionButton}</div>
                </div>
            </div>`; 
    }
    ...
    checkCaixaStatus(isInitialLoad = false) { 
        if (this.caixa.status === 'fechado') { 
            if (!isInitialLoad) { 
                this.showNotification('O caixa está fechado. É preciso abri-lo para iniciar um atendimento.', 'error'); 
    ...
    abrirCaixa(valorInicial) { 
        if (isNaN(valorInicial) || valorInicial < 0) { 
            this.showNotification('Valor inicial inválido.', 'error'); 
            return; 
        } 
        this.caixa = { 
            status: 'aberto', 
    ...
    showCaixaHistoryModal() { 
        const historyItems = this.caixaHistory
            .sort((a, b) => new Date(b.fechamento) - new Date(a.fechamento))
            .map(item => { 
                const differenceClass = item.diferenca < 0 ? 'text-red' : item.diferenca > 0 ? 'text-green' : ''; 
                return `
                    <div class="history-item">
                        <span><strong>Fechamento:</strong> ${this.formatDateTime(item.fechamento)}<br><strong>Faturamento:</strong> ${this.formatCurrency(item.totalEntradas)}</span>
                        <span class="${differenceClass}"><strong>Diferença:</strong><br>${this.formatCurrency(item.diferenca)}</span>
                    </div>`; 
            }).join(''); 
        const content = `
    ...
    renderPendingAppointments() { 
        ...
        if(!container) return; 
        const todayStr = this.getTodayDateString(); 
        const pending = this.appointments.filter(a => a.date === todayStr && a.status === 'pending'); 
        container.innerHTML = pending.length === 0 
            ? '<div class="empty-state"><div class="empty-icon">🧾</div><p>Nenhum atendimento pendente para hoje.</p></div>' 
    ...
    showFinalizeAppointmentModal(appointmentId) { 
        ...
        const content = `
            <form id="finalizeForm">
                ...
                <h4 style="margin-top: 1rem;">Total: ${this.formatCurrency(service.price)}</h4>
    ...
    showModal(title, content, options = {}) { 
        ...
        if(modalBackdropEl) modalBackdropEl.style.display = 'flex'; 
        if(modalContainerEl) modalContainerEl.style.maxWidth = options.width || '600px'; 
    }
    ...
    updateServiceDropdown() { 
        ...
        let optionsHTML = '<option value="">Selecione o serviço</option>' + sortedServices.map(s => `<option value="${s.id}" data-price="${s.price || 0}">${s.name} - ${this.formatCurrency(s.price || 0)}</option>`).join(''); 
        serviceSelects.forEach(select => { select.innerHTML = optionsHTML; }); 
    }
    ...
    updateBarberDropdowns() { 
        ...
        let optionsHTML = '<option value="">Selecione o profissional</option>' + sortedBarbers.map(b => `<option value="${b.id}">${b.name}</option>`).join(''); 
        barberSelects.forEach(select => { select.innerHTML = optionsHTML; }); 
    }
    ...
    showServiceModal(id = null) { 
        ...
        const content = `
            <form id="serviceForm">
                ...
                <div class="form-group"><label>Duração (minutos)</label><input type="number" step="15" name="duration" value="${service.duration || '45'}" required></div>
    ...
    saveService(serviceData) { 
        if (!serviceData.name || !serviceData.price) { 
            this.showNotification('Nome e preço do serviço são obrigatórios.', 'error'); 
            return; 
        } 
        if (serviceData.id) { 
    ...
    showClientDetailModal(phone) { 
        ...
        const totalSpent = clientAppointments.filter(a => a.status === 'completed').reduce((sum, a) => { 
            const service = this.services.find(s => s.id === a.service); 
            return sum + (service?.price || 0); 
        }, 0); 
        const lastVisitAppointment = clientAppointments.find(a => a.status === 'completed'); 
        let absenceString = 'Nenhuma visita concluída.'; 
        if (lastVisitAppointment) { 
            const lastVisitDate = new Date(lastVisitAppointment.date); 
            ...
            if (diffDays === 0) { 
                ...
                absenceString = `Há ${diffDays} dias`; 
            } 
        } 
        const historyHTML = clientAppointments.length > 0 
            ? clientAppointments.map(a => { 
                const service = this.services.find(s => s.id === a.service); 
                const statusMap = { scheduled: 'Agendado', completed: 'Concluído', cancelled: 'Cancelado', pending: 'Pendente' }; 
                return `<div class="history-item"> <span>${this.formatDate(a.date)} - ${service?.name || 'Serviço'}</span> <span>${this.formatCurrency(service?.price || 0)}</span> <span class="appointment-status status-${a.status}">${statusMap[a.status] || a.status}</span> </div>`; 
            }).join('') 
            : '<p class="empty-state" style="padding: 1rem 0;">Nenhum histórico de agendamento.</p>'; 
        const content = `
            <div class="client-details">
                ...
                <p><strong>Última Visita:</strong> ${absenceString}</p>
                <div class="separator" style="margin: 1rem 0;"></div>
    ...
    saveProfessional(professionalData) { 
        if (!professionalData.name || professionalData.name.trim() === '') { 
            this.showNotification('O nome do profissional é obrigatório.', 'error'); 
            return; 
        } 
        const existingProfessional = this.barbers.find(b => b.name.toLowerCase() === professionalData.name.toLowerCase() && b.id !== professionalData.id); 
        if (existingProfessional) { 
            this.showNotification('Já existe um profissional com este nome.', 'error'); 
            return; 
        } 
    ...
    deleteProfessional(id) { 
        if (this.barbers.length <= 1) { 
            this.showNotification('É necessário ter pelo menos um profissional cadastrado.', 'error'); 
            return; 
        } 
    ...
    saveExpense(expenseData) { 
        ...
        this.hideModal(); 
        this.loadDashboardData(); 
        if (document.getElementById('financialView')?.style.display === 'block') { 
            this.renderFinancialPage(); 
        } 
    ...
    showExpenseModal() { 
        ...
        const content = `
            <form id="expenseForm">
                <div class="form-group"><label>Descrição</label><input type="text" name="description" placeholder="Ex: Compra de lâminas" required></div>
        ...
        this.showModal('Adicionar Nova Despesa', content); 
        document.getElementById('expenseForm')?.addEventListener('submit', e => { 
            ...
            if (expenseData.description && expenseData.amount > 0) { 
                ...
                this.showNotification('Por favor, preencha todos os campos da despesa.', 'error'); 
            } 
        }); 
    }
    ...
    showAppointmentModal(isScheduled) { 
        ...
        if(clientSuggestionsEl) clientSuggestionsEl.innerHTML = this.clients.map(c => `<option value="${c.name}"></option>`).join(''); 
        const serviceOptions = this.services.map(s => `<option value="${s.id}">${s.name} - ${this.formatCurrency(s.price)}</option>`).join(''); 
        ...
        const agendaDateFilterValue = document.getElementById('agendaDateFilter')?.value || this.getTodayDateString(); 
        const dateAndTimeFields = isScheduled 
            ? ` <div class="form-group"><label>Data</label><input type="date" name="date" value="${agendaDateFilterValue}" required></div>
                <div class="form-group"><label>Horário</label><select name="time" required><option value="">Selecione</option>${timeOptions.join('')}</select></div>` 
            : ''; 
        const content = `
            <form id="barberBookingForm" data-walk-in="${!isScheduled}">
                <div class="form-group"><label>Cliente</label><input type="text" name="name" list="clientSuggestions" placeholder="Digite para buscar ou cadastrar novo" required></div>
                <div class="form-group"><label>Celular (com DDD)</label><input type="tel" name="phone" placeholder="11999999999" required></div>
                <div class="form-group"><label>Serviço</label><select name="service" required><option value="">Selecione</option>${serviceOptions}</select></div>
                <div class="form-group"><label>Profissional</label><select name="barber" required><option value="">Selecione</option>${barberOptions}</select></div>
                ${dateAndTimeFields}
                <div class="modal-footer">
                    <button type="button" class="btn-secondary" onclick="window.barberPro.hideModal()">Cancelar</button>
        ...
        nameInput?.addEventListener('input', () => { 
            const existingClient = this.clients.find(c => c.name === nameInput.value); 
            if (existingClient && phoneInput) { 
    ...
    navigateAgendaDays(direction) { 
        ...
        try { 
            ...
            console.error('Erro ao navegar entre datas:', e); 
            dateInput.value = this.getTodayDateString(); 
        } 
    }
    ...
    renderFullClientsList() { 
        ...
        container.innerHTML = clients.length === 0 
            ...
            : clients.map(client => { 
                ...
                const lastVisitAppointment = clientAppointments.filter(a => a.status === 'completed').sort((a,b) => b.date.localeCompare(a.date))[0]; 
                const lastVisit = lastVisitAppointment ? this.formatDate(lastVisitAppointment.date) : 'N/A'; 
                return `
                    <div class="data-card">
                        <div>
    ...
    renderCharts() { 
        ...
        if(revenueCtxEl) { 
            ...
            for (let i = 6; i >= 0; i--) { 
                ...
                weeklyData.labels.push(date.toLocaleDateString('pt-BR', { day: 'numeric', month: 'short' })); 
                const dailyRevenue = this.appointments
                    .filter(a => a.date === dateStr && a.status === 'completed')
                    .reduce((sum, a) => { 
                        const service = this.services.find(s => s.id === a.service); 
                        return sum + (service ? service.price : 0); 
                    }, 0); 
                weeklyData.data.push(dailyRevenue); 
            } 
            this.charts.revenueChart = new Chart(revenueCtx, { 
                type: 'bar', 
                data: { 
                    labels: weeklyData.labels, 
                    datasets: [{ label: 'Faturamento Diário', data: weeklyData.data, backgroundColor: 'rgba(199, 163, 85, 0.7)' }] 
                }, 
                options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true } } } 
            }); 
        } 
        const servicesCtxEl = document.getElementById('servicesChart'); 
        if(servicesCtxEl) { 
            const servicesCtx = servicesCtxEl.getContext('2d'); 
            const serviceAnalytics = this.services
                .map(s => { 
                    const appointments = this.appointments.filter(a => a.service === s.id && a.status === 'completed'); 
                    return { name: s.name, count: appointments.length, revenue: appointments.length * s.price }; 
                })
                .filter(s => s.count > 0)
                .sort((a, b) => b.revenue - a.revenue); 
            this.charts.servicesChart = new Chart(servicesCtx, { 
                type: 'bar', 
                data: { 
                    labels: serviceAnalytics.map(s => s.name), 
                    datasets: [{ label: 'Faturamento por Serviço', data: serviceAnalytics.map(s => s.revenue), backgroundColor: 'rgba(59, 130, 246, 0.7)' }] 
                }, 
    ...
    renderFinancialPage() {
        ...
        const addExpenseBtn = document.getElementById('addExpenseBtnMain');

        if (!container || !openCaixaBtn || !closeCaixaBtn || !addExpenseBtn) return;

        if (this.caixa.status === 'aberto') {
            openCaixaBtn.style.display = 'none';
            closeCaixaBtn.style.display = 'inline-block';
            ...
            const transactions = [...this.caixa.entradas, ...this.caixa.saidas]
                .sort((a, b) => new Date(b.data) - new Date(a.data))
                .slice(0, 10); 

            const transactionsHTML = transactions.length > 0 ? transactions.map(t => {
                const isEntrada = 'metodo' in t;
                return `<div class="transaction-item ${isEntrada ? 'entrada' : 'saida'}">
                    <span>${new Date(t.data).toLocaleTimeString('pt-BR')} - ${t.descricao}</span>
                    <span>${isEntrada ? '' : '-'}${this.formatCurrency(t.valor)}</span>
                </div>`;
            }).join('') : '<p class="empty-state">Nenhuma transação hoje.</p>';

            container.innerHTML = `
                <div class="financial-summary">
                    <div class="summary-item">
                        <p>Saldo Inicial</p>
                        <h4>${this.formatCurrency(this.caixa.saldoInicial)}</h4>
                    </div>
                    <div class="summary-item">
                        <p>Entradas (Hoje)</p>
                        <h4 class="text-green">${this.formatCurrency(totalEntradas)}</h4>
                    </div>
                    <div class="summary-item">
                        <p>Saídas (Hoje)</p>
                        <h4 class="text-red">${this.formatCurrency(totalSaidas)}</h4>
                    </div>
                    <div class="summary-item total">
                        <p>Saldo Atual</p>
                        <h4>${this.formatCurrency(saldoAtual)}</h4>
                    </div>
                </div>
                <h4 style="margin-top: 2rem;">Últimas Transações</h4>
                <div class="transactions-list">${transactionsHTML}</div>
            `;
        } else {
            openCaixaBtn.style.display = 'inline-block';
            closeCaixaBtn.style.display = 'none';
            ...
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <h3>Caixa Fechado</h3>
                    <p>Clique em "Abrir Caixa" para iniciar as operações do dia.</p>
                </div>
            `;
        }
    }
