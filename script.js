// BarberPro - Versão 2.4 com Correção de Bugs
class BarberPro {
    constructor() {
    ...
    setupEventListeners() {
        ...
        safeAddEventListener('addExpenseBtnMain', 'click', () => this.showExpenseModal()); // Event listener for the expense button on the financial page
    ...
    renderDashboardAppointments() { const appointmentsListEl = document.getElementById('appointmentsList'); if(!appointmentsListEl) return; const today = this.getTodayDateString(); const upcomingAppointments = this.appointments.filter(a => a.date >= today && (a.status === 'scheduled' || a.status === 'pending')).sort((a,b) => (a.date + a.time).localeCompare(b.date + b.time)); appointmentsListEl.innerHTML = upcomingAppointments.length === 0 ? '<div class="empty-state"><div class="empty-icon">📅</div><p>Nenhum agendamento futuro.</p></div>' : upcomingAppointments.map(a => this.createAppointmentCard(a)).join(''); }
    ...
    renderAgendaView() {
        ...
        let timeColumnHTML = '<div class="agenda-time-column">' + timeSlots.map(time => `<div class="agenda-time-slot">${time.endsWith(':00') ? `<b>${time}</b>` : time}</div>`).join('') + '</div>';
