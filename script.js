...
class BarberPro {
    constructor() {
        ...
        this.charts = {};
        this.DATA_VERSION = '2.1'; // Versão incrementada para o histórico
    ...
    // ... (As funções showSection, switchDashboardView, etc., continuam iguais, mas a versão completa está aqui)
    ...
    // (As funções restantes estão aqui para garantir que o código esteja completo)
    saveService(serviceData) {
        if (!serviceData.name || !serviceData.price || !serviceData.duration) { this.showNotification('Todos os campos são obrigatórios!', 'error'); return; }
        serviceData.price = parseFloat(serviceData.price);
        serviceData.duration = parseInt(serviceData.duration);
        if (isNaN(serviceData.price) || serviceData.price <= 0 || isNaN(serviceData.duration) || serviceData.duration <= 0) { this.showNotification('Por favor, insira um preço e duração válidos!', 'error'); return; }
        if (serviceData.id) {
            const index = this.services.findIndex(s => s.id === serviceData.id);
            if (index !== -1) { this.services[index] = { ...this.services[index], ...serviceData }; this.showNotification('Serviço atualizado com sucesso!', 'success'); }
        } else { const newId = `s_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`; this.services.push({ ...serviceData, id: newId }); this.showNotification('Serviço adicionado com sucesso!', 'success'); }
        this.saveData();
        this.hideModal();
        this.renderServicesList();
        this.updateServiceDropdown();
        this.renderClientServices();
        if (document.getElementById('reportsView')?.classList.contains('active')) { this.renderCharts(); }
    }
    ...
    showFecharCaixaModal() {
        if (this.caixa.status === 'fechado') { this.showNotification('O caixa já está fechado.', 'error'); return; }
        const totalsByMethod = this.caixa.entradas.reduce((acc, entrada) => { acc[entrada.metodo] = (acc[entrada.metodo] || 0) + entrada.valor; return acc; }, {});
        ...
        countedCashInput?.addEventListener('input', () => {
            ...
            if (difference < 0) { diffValorEl.className = 'text-red'; } else if (difference > 0) { diffValorEl.className = 'text-green'; } else { diffValorEl.className = ''; }
        });
