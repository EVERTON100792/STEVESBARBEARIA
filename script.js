// BarberPro - Versão FINAL - Todas as correções aplicadas
class BarberPro {
    constructor() {
    ...
    // 1. Inicialização e Renderização Principal
    init() {
        try {
    ...
    // 2. Gerenciador Global de Eventos (Cliques, Envios de Formulário, etc.)
    setupGlobalEventListeners() {
        ...
        document.body.addEventListener('submit', e => {
            ...
            switch(form.id){
                case 'bookingForm':
                    this.handleAppointmentSubmit(new FormData(form), false);
                    form.reset();
                    break;
    ...
    showSection(sectionId) {
        ...
        if(clientAreaBtn && barberAreaBtn){
            ...
            barberAreaBtn.classList.toggle('active', sectionId === 'barberLogin');
        }
    }
    ...
    // 4. Gerenciamento de Dados (LocalStorage)
    checkDataVersion() { if (localStorage.getItem('barberpro_data_version') !== this.DATA_VERSION) { localStorage.clear(); localStorage.setItem('barberpro_data_version', this.DATA_VERSION); } }
    ...
    resetTestData() { if (confirm('ATENÇÃO!\nIsto irá apagar TODOS os agendamentos, clientes e dados financeiros.\nDeseja continuar?')) { this.appointments = []; this.clients = []; this.caixa = { status: 'fechado', saldoInicial: 0, abertura: null, entradas: [], saidas: [] }; this.caixaHistory = []; this.saveData(); this.loadDashboardData(); this.switchDashboardView('dashboardView'); this.showNotification('Dados de teste foram zerados.', 'success'); } }
    
    // 5. Autenticação
    checkAuthentication() { const token = this.getFromStorage('auth_token', null); if (token) { this.currentUser = token; this.renderBarberView(); } else { this.showSection('barberLogin'); } }
    login(username, password) { if (username === 'barbeiro' && password === '123') { this.currentUser = { username: 'barbeiro' }; this.saveToStorage('auth_token', this.currentUser); this.renderBarberView(); this.showNotification('Login realizado com sucesso!', 'success'); return true; } return false; }
    logout() { this.currentUser = null; localStorage.removeItem('barberpro_auth_token'); this.renderClientView(); this.showNotification('Você saiu da sua conta.'); }
    handleLoginSubmit(form) { const usernameEl = form.querySelector('#username'); const passwordEl = form.querySelector('#password'); if (usernameEl && passwordEl) { if (!this.login(usernameEl.value, passwordEl.value)) { this.showNotification('Usuário ou senha inválidos.', 'error'); } } }
    
    // 6. Lógica de Agendamentos e Comandas
    handleAppointmentSubmit(formData, isWalkIn) {
        const name = formData.get('name').trim(); const phone = this.sanitizePhone(formData.get('phone'));
        if (!name || phone.length < 10 || !formData.get('service') || !formData.get('barber')) { return this.showNotification('Por favor, preencha todos os campos obrigatórios.', 'error'); }
        ...
        const appointment = { id: Date.now().toString(), createdAt: new Date().toISOString(), name, phone, service: formData.get('service'), barberId: formData.get('barber'), date: appointmentDate, time: appointmentTime, status: isWalkIn ? 'in_progress' : 'scheduled', paymentStatus: 'pending' };
    ...
    showComandaActionsModal(appointmentId) { const appointment = this.appointments.find(a => a.id === appointmentId); if (!appointment) return; const service = this.services.find(s => s.id === appointment.service); const barber = this.barbers.find(b => b.id === appointment.barberId); let actions = ''; if (appointment.status === 'scheduled') { actions = `<button id="startServiceBtn" class="btn-primary">Iniciar Atendimento</button>`; } else if (appointment.status === 'in_progress') { actions = `<button id="finalizeServiceBtn" class="btn-primary">Finalizar e Receber</button>`; } const content = `<div><p><strong>Cliente:</strong> ${appointment.name}</p><p><strong>Serviço:</strong> ${service?.name || 'N/A'}</p><p><strong>Profissional:</strong> ${barber?.name || 'N/A'}</p><p><strong>Horário:</strong> ${this.formatDate(appointment.date)} às ${appointment.time}</p><p><strong>Valor:</strong> ${this.formatCurrency(service?.price || 0)}</p></div><div class="modal-footer"><button type="button" class="btn-secondary" onclick="window.barberPro.hideModal()">Cancelar</button>${actions}</div>`; this.showModal('Gerenciar Atendimento', content); const startBtn = document.getElementById('startServiceBtn'); const finalizeBtn = document.getElementById('finalizeServiceBtn'); if(startBtn) startBtn.addEventListener('click', () => this.startService(appointmentId)); if(finalizeBtn) finalizeBtn.addEventListener('click', () => this.showFinalizeAppointmentModal(appointmentId)); }
    startService(appointmentId) { if (!this.checkCaixaStatus()) return; const appointment = this.appointments.find(a => a.id === appointmentId); if (appointment) { appointment.status = 'in_progress'; this.saveData(); this.hideModal(); this.showNotification('Atendimento iniciado!', 'success'); this.renderRelevantViews(); } }
    showFinalizeAppointmentModal(appointmentId) { const appointment = this.appointments.find(a => a.id === appointmentId); if (!appointment) return; const service = this.services.find(s => s.id === appointment.service); const content = `<form id="finalizeForm"><p>Finalizando atendimento de <strong>${appointment.name}</strong>.</p><h4>Total: ${this.formatCurrency(service?.price || 0)}</h4><div class="form-group" style="margin-top:1.5rem"><label for="paymentMethod">Forma de Pagamento</label><select id="paymentMethod" name="paymentMethod" required><option value="Dinheiro">Dinheiro</option><option value="Pix">Pix</option><option value="Cartão de Débito">Cartão de Débito</option><option value="Cartão de Crédito">Cartão de Crédito</option></select></div><div class="modal-footer"><button type="button" class="btn-secondary" onclick="window.barberPro.hideModal()">Cancelar</button><button type="submit" class="btn-primary">Confirmar Pagamento</button></div></form>`; this.showModal('Finalizar e Receber', content); const finalizeForm = document.getElementById('finalizeForm'); if(finalizeForm) finalizeForm.addEventListener('submit', e => { e.preventDefault(); const paymentMethod = e.target.querySelector('[name="paymentMethod"]').value; this.finalizeAppointment(appointmentId, paymentMethod); }); }
    finalizeAppointment(appointmentId, paymentMethod) { const appointment = this.appointments.find(a => a.id === appointmentId); if (!appointment) return; const service = this.services.find(s => s.id === appointment.service); if (!service) return; this.caixa.entradas.push({ descricao: `Serviço: ${service.name} (${appointment.name})`, valor: service.price, data: new Date().toISOString(), metodo: paymentMethod }); appointment.status = 'completed'; appointment.paymentStatus = 'paid'; appointment.paymentMethod = paymentMethod; appointment.closedAt = new Date().toISOString(); this.saveData(); this.hideModal(); this.showNotification(`Pagamento de ${this.formatCurrency(service.price)} recebido!`, 'success'); this.renderRelevantViews(); }
    cancelAppointment(id) { if (confirm('Tem certeza que deseja cancelar este agendamento?')) { const appointment = this.appointments.find(a => a.id === id); if (appointment) { appointment.status = 'cancelled'; this.saveData(); this.renderRelevantViews(); this.showNotification('Agendamento cancelado.'); } } }
    
    // 7. Lógica do Caixa
    checkCaixaStatus(isInitialLoad = false) { if (this.caixa.status === 'fechado') { if (!isInitialLoad) { this.showNotification('O caixa está fechado. É preciso abri-lo para esta operação.', 'error'); this.showAbrirCaixaModal(); } return false; } return true; }
    abrirCaixa(valorInicial) { if (isNaN(valorInicial) || valorInicial < 0) return this.showNotification('Valor inicial inválido.', 'error'); this.caixa = { status: 'aberto', saldoInicial: valorInicial, abertura: new Date().toISOString(), entradas: [], saidas: [] }; this.saveData(); this.hideModal(); this.updateCaixaStatusIndicator(); this.renderFinancialPage(); this.showNotification(`Caixa aberto com ${this.formatCurrency(valorInicial)}!`, 'success'); }
    fecharCaixa() { const countedCashEl = document.getElementById('countedCash'); if (!countedCashEl) return; const countedCash = parseFloat(countedCashEl.value) || 0; const totalsByMethod = this.caixa.entradas.reduce((acc, e) => ({ ...acc, [e.metodo]: (acc[e.metodo] || 0) + e.valor }), {}); const totalEntradas = this.caixa.entradas.reduce((sum, item) => sum + item.valor, 0); const totalSaidas = this.caixa.saidas.reduce((sum, item) => sum + item.valor, 0); const dinheiroEntradas = totalsByMethod['Dinheiro'] || 0; const saldoEsperadoDinheiro = this.caixa.saldoInicial + dinheiroEntradas - totalSaidas; const closingSummary = { id: `caixa_${Date.now()}`, abertura: this.caixa.abertura, fechamento: new Date().toISOString(), saldoInicial: this.caixa.saldoInicial, totalEntradas, totalSaidas, saldoEsperadoDinheiro, dinheiroConferido: countedCash, diferenca: countedCash - saldoEsperadoDinheiro }; this.caixaHistory.push(closingSummary); this.caixa = { status: 'fechado', saldoInicial: 0, abertura: null, entradas: [], saidas: [] }; this.saveData(); this.hideModal(); this.updateCaixaStatusIndicator(); this.renderFinancialPage(); this.showNotification('Caixa fechado e salvo no histórico.', 'success'); }
    
    // 8. Funções de Renderização de Views do Dashboard
    ...
    renderFinancialPage() { const container = document.getElementById('financialDashboard'); const openBtn = document.getElementById('openCaixaBtn'); const closeBtn = document.getElementById('closeCaixaBtn'); const expenseBtn = document.getElementById('addExpenseBtnMain'); if (!container || !openBtn || !closeBtn || !expenseBtn) return; if (this.caixa.status === 'aberto') { openBtn.style.display = 'none'; closeBtn.style.display = 'inline-block'; expenseBtn.style.display = 'inline-block'; const totalEntradas = this.caixa.entradas.reduce((sum, e) => sum + e.valor, 0); const totalSaidas = this.caixa.saidas.reduce((sum, s) => sum + s.valor, 0); const saldoAtual = this.caixa.saldoInicial + totalEntradas - totalSaidas; container.innerHTML = `<div class="financial-summary"><div class="summary-item"><p>Saldo Inicial</p><h4>${this.formatCurrency(this.caixa.saldoInicial)}</h4></div><div class="summary-item"><p>Entradas</p><h4 class="text-green">${this.formatCurrency(totalEntradas)}</h4></div><div class="summary-item"><p>Saídas</p><h4 class="text-red">${this.formatCurrency(totalSaidas)}</h4></div><div class="summary-item total"><p>Saldo Atual</p><h4>${this.formatCurrency(saldoAtual)}</h4></div></div>`; } else { openBtn.style.display = 'inline-block'; closeBtn.style.display = 'none'; expenseBtn.style.display = 'none'; container.innerHTML = `<div class="empty-state"><h3>Caixa Fechado</h3><p>Clique em "Abrir Caixa" para iniciar as operações.</p></div>`; } }
    renderCharts() { /* Em construção */ }
    ...
    updateServiceDropdown() { const selects = document.querySelectorAll('select[name="service"]'); if(selects.length === 0) return; const optionsHTML = '<option value="">Selecione o serviço</option>' + this.services.map(s => `<option value="${s.id}">${s.name} - ${this.formatCurrency(s.price)}</option>`).join(''); selects.forEach(select => select.innerHTML = optionsHTML); }
    updateBarberDropdowns() { const selects = document.querySelectorAll('select[name="barber"]'); if(selects.length === 0) return; const optionsHTML = '<option value="">Selecione o profissional</option>' + this.barbers.map(b => `<option value="${b.id}">${b.name}</option>`).join(''); selects.forEach(select => select.innerHTML = optionsHTML); }
    ...
    navigateAgendaDays(direction) { const dateInput = document.getElementById('agendaDateFilter'); if (!dateInput || !dateInput.value) return; const dateParts = dateInput.value.split('-').map(Number); const currentDate = new Date(Date.UTC(dateParts[0], dateParts[1] - 1, dateParts[2])); currentDate.setUTCDate(currentDate.getUTCDate() + direction); dateInput.value = currentDate.toISOString().split('T')[0]; this.renderAgendaView(); }
