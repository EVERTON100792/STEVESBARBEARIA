class BarberPro {
    constructor() {
        ...
        this.charts = {};
        this.DATA_VERSION = '6.9'; // Version updated
    ...
    setupGlobalEventListeners() {
            ...
            if (buttonTarget) {
                ...
                switch (buttonId) {
                    ...
                    case 'addServiceBtn':
                        ...
                        break;
                    case 'addProfessionalBtn':
                        this.showProfessionalModal();
                        break;
                    case 'addAppointmentBtn':
                    case 'quickAddAppointmentBtn':
                        this.showAppointmentModal(true);
                        break;
                    case 'addWalkInBtn':
                        this.showAppointmentModal(false);
                        break;
                    case 'welcomeOpenCaixaBtn':
                    case 'openCaixaBtn':
                        this.showAbrirCaixaModal();
                        break;
                    case 'closeCaixaBtn':
                        this.showFecharCaixaModal();
                        break;
                    case 'viewHistoryBtn':
                        this.showCaixaHistoryModal();
                        break;
                    case 'addExpenseBtnMain':
                        this.showExpenseModal();
                        break;
                    case 'closeModalBtn':
                        this.hideModal();
                        break;
                    case 'lookupAppointmentsBtn':
                        this.lookupClientAppointments();
                        break;
                    case 'newBookingBtn':
                        this.toggleClientView(true);
                        break;
                    case 'backToHomeBtn':
                        this.toggleClientView(false);
                        break;
                }
            }
        ...
        document.body.addEventListener('submit', e => {
            ...
            switch (form.id) {
                case 'clientDetailsForm':
                    this.handleClientBookingConfirmation(e);
                    break;
                ...
                case 'expenseForm':
                    ...
                    break;
                case 'openCaixaForm':
                    this.abrirCaixa(this.parseCurrency(new FormData(form).get('initialValue')));
                    break;
                case 'closeCaixaForm':
                    this.fecharCaixa(this.parseCurrency(new FormData(form).get('countedCash')));
                    break;
            }
        });
        ...
        document.body.addEventListener('input', e => {
            ...
            if (e.target.id === 'serviceSearchInput') this.renderClientServices();
...
<button type="button" class="btn-primary" onclick="window.barberPro.saveProfessional(new FormData(document.getElementById('professionalForm')))">${id ? 'Salvar Alterações' : 'Adicionar Profissional'}</button>
    ...
    saveProfessional(formData) {
        ...
        if (id) {
            ...
            if (index !== -1) {
                this.barbers[index] = { ...this.barbers[index],
                    ...professionalData
                };
    ...
    renderClientView() {
        ...
        this.toggleClientView(false); // Garante que a visão inicial seja mostrada
        this.resetBookingFlow();
...
`).join('');
    ...
    toggleClientView(showServices) {
        const initialView = document.getElementById('initialClientView');
        const servicesView = document.getElementById('clientServicesSection');
        if (initialView && servicesView) {
            initialView.style.display = showServices ? 'none' : 'block';
            servicesView.style.display = showServices ? 'block' : 'none';
        }
    }

    lookupClientAppointments() {
        const phoneInput = document.getElementById('clientPhoneLookup');
        const resultContainer = document.getElementById('clientAppointmentsResult');
        if (!phoneInput || !resultContainer) return;

        const phone = this.sanitizePhone(phoneInput.value);
        if (phone.length < 10) {
            this.showNotification('Por favor, insira um número de celular válido com DDD.', 'error');
            return;
        }
        ...
        if (clientAppointments.length > 0) {
            resultContainer.innerHTML = `<h4>Seus Agendamentos</h4>` + clientAppointments.map(a => this.createAppointmentCard(a, false)).join('');
        } else {
            resultContainer.innerHTML = `<p class="empty-state">Nenhum agendamento encontrado para este número.</p>`;
        }
    }
    ...
    resetBookingFlow() {
        this.bookingState = {};
        this.toggleClientView(false); // Volta para a tela inicial do cliente
...
`).join('');
    ...
    renderConfirmationStep() {
        const { serviceName, barberName, servicePrice, time } = this.bookingState;
        const date = document.getElementById('bookingDate').value;
...
`;
    ...
    showSection(sectionId) {
        ...
        if (navbar) {
            // Mostra a navbar apenas na área do cliente, esconde no login/dashboard do barbeiro
            navbar.style.display = sectionId === 'clientArea' ? 'block' : 'none';
        }
    }
    ...
    switchDashboardView(viewId) {
        ...
        const titles = {
            ...
            appointmentsView: {
                title: 'Agenda',
                subtitle: 'Visualize e gerencie seus horários.'
            },
            comandasView: {
                title: 'Comandas Abertas',
                subtitle: 'Gerencie os atendimentos em andamento.'
            },
            clientsView: {
                title: 'Clientes',
                subtitle: 'Consulte e gerencie sua base de clientes.'
            },
            servicesView: {
                title: 'Serviços',
                subtitle: 'Configure os serviços oferecidos.'
            },
            professionalsView: {
                title: 'Profissionais',
                subtitle: 'Gerencie a equipe de barbeiros.'
            },
            financialView: {
                title: 'Caixa',
            ...
            reportsView: {
                title: 'Relatórios',
                subtitle: 'Analise o desempenho da sua barbearia.'
            }
        };
    ...
    renderDashboardAppointments() {
        ...
        listEl.innerHTML = upcoming.length === 0 ? '<p class="empty-state">Nenhum agendamento futuro.</p>' : upcoming.map(a => this.createAppointmentCard(a, true)).join('');
    }
    renderPendingAppointments() {
        ...
        container.innerHTML = pending.length === 0 ? '<p class="empty-state">Nenhuma comanda em andamento.</p>' : pending.map(app => this.createAppointmentCard(app, true)).join('');
    }
    renderAgendaView() {
...
`;
    ...
    createAppointmentCard(appointment, isClickable = true) {
        const service = this.services.find(s => s.id === appointment.service);
